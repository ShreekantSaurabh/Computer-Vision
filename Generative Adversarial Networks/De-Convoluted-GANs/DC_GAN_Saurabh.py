...
import torchvision.datasets as dset         #torchvision is used for visualising the images
...
# Creating the transformations (for generator)
transform = transforms.Compose([transforms.Scale(imageSize), transforms.ToTensor(), transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5)),]) # We create a list of transformations (scaling, tensor conversion, normalization) to apply to the input images.

# Loading the dataset
dataset = dset.CIFAR10(root = './data', download = True, transform = transform) # We download the training set in the ./data folder and we apply the previous transformations on each image.
dataloader = torch.utils.data.DataLoader(dataset, batch_size = batchSize, shuffle = True, num_workers = 2) # We use dataLoader to get the images of the training set batch by batch. shuffle = True for randomise the images. num_workers = 2 is no. of threads=2
...
class G(nn.Module): # We introduce a class to define the generator.
    ...
    def __init__(self): # We introduce the __init__() function that will define the architecture of the generator.
        super(G, self).__init__() # We inherit __init__ from the nn.Module tools.
        self.main = nn.Sequential( # We create a meta module of a neural network that will contain a sequence of several modules (convolutions, full connections, etc.).
            nn.ConvTranspose2d(100, 512, 4, 1, 0, bias = False), # We start with an inversed convolution ie. ConvTranspose2d.
            #size of input to ConvTranspose2d = 100, no. of feature maps / output = 512, size of kernel = 4, stride = 1, padding=0
            nn.BatchNorm2d(512), # We normalize all the features along the dimension of the batch = 512
            nn.ReLU(True), # We apply a ReLU rectification to break the linearity.
            nn.ConvTranspose2d(512, 256, 4, 2, 1, bias = False), # We add another inversed convolution.
            #output of 1st ConvTranspose2d becomes the input for 2nd ConvTranspose2d.
...
criterion = nn.BCELoss() # We create a criterion object that will measure the error between the prediction and the target. BCE is Binary Cross Entropy. To check these parameters, please visit pytorch.org/docs/master
...
for epoch in range(25): # We iterate over 25 epochs.
    ...
    for i, data in enumerate(dataloader, 0): # We iterate over the images of the dataset.
        ...
        target = Variable(torch.ones(input.size()[0])) # We get the target as 1 since all images are real.
        ...
        noise = Variable(torch.randn(input.size()[0], 100, 1, 1)) # We make a random input vector (noise) of the generator. input vector = 100 of size 1*1 
        ...
        target = Variable(torch.zeros(input.size()[0])) # We get the target as 0 since all images are fake.
